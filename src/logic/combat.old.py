...
# class Combate:
#     def __init__(self, personagem, inimigos, conn):
...
#     def fugir(self):
#         debug(f"chance de fuda inimigo: {self.inimigo}")
...
#         if all(inimigo.vida <= 0 for inimigo in self.inimigos):
#             self.combate_ativo = False
#             print(Style.BRIGHT + Fore.GREEN + "Todos os inimigos foram derrotados..." + Style.RESET_ALL)
...
#                 if inimigo.vida > 0:
#                     print(Fore.RED + f"{inimigo.nome}: {inimigo.vida} HP" + Style.RESET_ALL)
#             print(Style.BRIGHT + Fore.MAGENTA + "---------------------------------------------------------------" + Style.RESET_ALL)
...
#                 id = int(input(Style.BRIGHT + Fore.RED + "Escolher o inimigo: " + Style.RESET_ALL))
#                 self.inimigo = self.inimigos[id - 1]
#                 while self.inimigo not in self.inimigos:
#                     id = input(Style.BRIGHT + Fore.RED + "Escolher o inimigo: " + Style.RESET_ALL)
#                     self.inimigo = self.inimigos[id - 1]
...
#         # If every enemy is dead, the combat is over:
#         if all(inimigo.vida <= 0 for inimigo in self.inimigos):
...
# def verificar_percepcao(personagem, inimigo):
